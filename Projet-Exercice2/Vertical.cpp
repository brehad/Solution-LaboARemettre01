...
void main()																			
{
	...
	int c1;																			//FR : Les variables de c1 à c5 servent à stocker chacuns des chiffres indépendemment
	int c2;
	int c3;
	int c4;
	int c5;
	...
	if (nb>9999 && nb<100000)														//Vérifie si le nombre nb est entre 10000 et 99999. Le && sert à ajouter une condition.
	{
		...
		c4 = ((nb % 100-c5) / 10);													//FR : Avec cette stratégie et en soustrayant et divisant les nombres j'ai réussi à isoler chacuns des chiffres séparément.
		...
		std::cout << c1 << endl;													//FR : Le programme affiche la valeur de chacunes des variables dans l'ordre. endl sert à sauter une ligne
		std::cout << c2 << endl;
		std::cout << c3 << endl;
		std::cout << c4 << endl;
		std::cout << c5 << endl;
	...
	{
		c5 = abs(nb % -10);															//FR : Ce bloc est presque identique à celui d'avant. La seule différence est que j'ai mis des valeur absolue pour obtenir des chiffres positifs.
	...
	{
		std::cout << "Ce nombre ne contient pas 5 chiffres.";						//FR : Si le nombre entré ne corresponds pas aux conditions plus haut, le programme affiche dans la console que le nombre ne contient pas 5 chiffres.
	}
